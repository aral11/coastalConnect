...
export function SupabaseAuthProvider({ children }: { children: ReactNode }) {
  ...
  const signIn = async (email: string, password: string) => {
    ...
    try {
      ...
      await trackEvent('sign_in_attempt', { email, success: false, error: error.message });
      throw error;
    } finally {
      setLoading(false);
    }
  ...
  const signUp = async (email: string, password: string, userData: Partial<AuthUser>) => {
    ...
    try {
      ...
      await trackEvent('sign_up_attempt', { email, role: userData.role, success: true });
    } catch (error: any) {
      await trackEvent('sign_up_attempt', { email, success: false, error: error.message });
      throw error;
    } finally {
      setLoading(false);
    }
