...
export default function PaymentGateway({
  ...
  const loadPaymentMethods = async () => {
    try {
      const response = await fetch(`/api/payments/methods?currency=${currency}&country=IN`);
      const data = await response.json();
      
      if (data.success) {
        setPaymentMethods(data.data);
        // Set default gateway based on currency
        if (currency === 'INR' && data.data.find(m => m.gateway === 'razorpay')) {
  ...
  const processRazorpayPayment = async () => {
    try {
      ...
      const options = {
        key: process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID || 'rzp_test_9WgjuKhbMPLvB4', // Use test key for demo
        amount: order.amount,
      ...
      <CardContent className="space-y-6">
        ...
        {paymentMethods.length > 1 && (
          <div className="space-y-3">
            <div className="text-sm font-medium text-gray-900">Select Payment Method</div>
...
export function PaymentSuccess({ paymentData, onClose }: { 
  paymentData: any; 
  onClose: () => void;
}) {
  return (
